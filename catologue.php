...
$args = "SELECT *, products.id as productId, suppliers.id as supplierId FROM products JOIN suppliers on products.posted_by=suppliers.id WHERE suppliers.isActive = 1";
$result = mysqli_query($conn, $args) or die("database error:". mysqli_error($conn));

// check no of data
$result1 = mysqli_query($conn, "SELECT COUNT(products.id) AS count FROM products JOIN suppliers on products.posted_by=suppliers.id WHERE suppliers.isActive = 1");
...
<?php $pageTitle = "Catologue"; include('header.php');?>

  <section>
    <div class="container p-5">
    <div class="row g-0">
          <div class="col-sm-6 col-md-8"><h1>Shop</h1></div>
    ...
    <div class="container mb-3">
      <?php if($count == 0){ ?>
        <div class="alert alert-secondary" role="alert">
          There is no products for listing. Check back later!
        </div>
      <?php }else{ ?>
